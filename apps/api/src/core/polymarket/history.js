import { fetchClobPriceHistoryBatch } from './clob.js';
import { fetchPolymarketTradesForMarkets } from './data-api.js';
...
function normalizeContract(bucket, stationId = null) {
...
function buildBoardSnapshot(timestamp, contracts, priceHistoryByTokenId, trades, eventDate, stationId = null) {
  return {
    stationId,
    eventDate,
    timestamp,
...
export function buildHistoricalBoardArchiveFromBuckets(bucketMarkets, {
  ...
  const contracts = (Array.isArray(bucketMarkets) ? bucketMarkets : [])
    .map((bucket) => normalizeContract(bucket, stationId))
    .filter(Boolean)
  ...
  for (const timestamp of boardTimes) {
    for (const [eventDate, dateContracts] of contractsByDate.entries()) {
      boardSnapshots.push(buildBoardSnapshot(timestamp, dateContracts, normalizedPriceHistory, normalizedTrades, eventDate, stationId));
    }
  }
